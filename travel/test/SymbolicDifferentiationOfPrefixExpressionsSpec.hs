...
spec = do
  ...
  it "Nested expressions" $ do
    ...
    when (result/="(* 2 (* -1 (sin (* 2 x))))" && result/="(* -2 (sin (* 2 x)))") $
      expectationFailure $ "`diff \"(cos (* 2 x))\"`: expected either \"(* 2 (* -1 (sin (* 2 x))))\" or \"(* -2 (sin (* 2 x)))\", but got \"" ++ result ++ "\""
  ...
  it "Second derivatives" $ do
    ...
    when (result/="(* 3 (* 2 x))" && result/="(* 6 x)") $
      expectationFailure $ "`diff (diff \"(^ x 3)\")`: expected either \"(* 3 (* 2 x))\" or \"(* 6 x)\", but got \"" ++ result ++ "\""